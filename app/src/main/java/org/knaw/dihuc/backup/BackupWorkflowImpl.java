...
public class BackupWorkflowImpl implements BackupWorkflow {
  ...
  private final BackupActivities defaultTaskQueueStore;

  public BackupWorkflowImpl() {
    // Create activity clients.
    ActivityOptions ao =
        ActivityOptions.newBuilder()
            .setScheduleToCloseTimeout(Duration.ofSeconds(10))
            .setTaskQueue(BackupWorker.TASK_QUEUE)
            .build();
    this.defaultTaskQueueStore = Workflow.newActivityStub(BackupActivities.class, ao);
  }

  @Override
  public void processFile(File fileset) {
    ...
    Workflow.retry(
        ...
        () -> processFileImpl(fileset));
  }

  private void processFileImpl(File fileset) {
    final BackupActivities.TaskQueueFileNamePair backup = defaultTaskQueueStore.bindTasksToHost(fileset);

    // Now initialize stubs that are specific to the returned task queue.
    final ActivityOptions hostActivityOptions =
        ActivityOptions.newBuilder()
            .setTaskQueue(backup.getHostTaskQueue())
            .setScheduleToCloseTimeout(Duration.ofSeconds(10))
            .build();
    final BackupActivities hostSpecificStore =
        Workflow.newActivityStub(BackupActivities.class, hostActivityOptions);

    hostSpecificStore.checksum(fileset);
    hostSpecificStore.rsync(fileset);
    hostSpecificStore.move(fileset);
  }
}
